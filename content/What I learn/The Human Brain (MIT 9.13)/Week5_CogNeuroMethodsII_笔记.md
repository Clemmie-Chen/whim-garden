...
created: 2026-06-03T15:37:33.207+08:00
modified: 2026-06-03T15:37:33.207+08:00
tags:
  - cog-neuro-sci
---
...
⭐ 核心计算难题：每次看到同一张脸，它在光照、朝向、发型、情绪等方面都不一样（each time we see a face it looks different in lighting, orientation, hair, mood, etc.），由此产生**单张人脸跨图像的巨大变异**（huge variation across images of a single face）。这是问题 1 的答案。

# B. 行为（复习）

行为证据已表明：对不熟悉的人，我们不擅长提取不变表征（we are bad at extracting invariant representations of unfamiliar people）。

**倒脸效应（face inversion effect，Yin, 1969）**——本系 1969 年一篇博士论文中的发现：相比正立刺激，上下颠倒刺激的识别错误增多（an increase in errors for upside-down compared to upright stimuli）；且人脸的倒置效应大于房子、火柴人（stick figures）等其它刺激。提示人脸识别可能与物体识别工作方式不同。

⭐ 问题 2 的答案（来自简单行为数据）：人类从人脸提取的表征是非图像不变的（not image invariant，对不熟悉人脸而言）、朝向特异的（orientation specific）。

**行为方法的优缺点（Strengths and Weaknesses of Behavioral Methods）**

优点：(1) 善于刻画内部表征（至少定性 at least qualitatively）；(2) 善于分离不同的心理现象（如人脸 vs 物体加工）；(3) 便宜。
...
假设：脑中存在一个对人脸选择性反应的区域。检验：把人放进扫描仪，看人脸与物体。是否找到对人脸 > 物体反应更高的区域？找到了。

**先想替代假说，再逐一检验（think up alternative hypotheses, then test them）**

该反应是否有更简单的解释？它是否对——任何人类相关之物（anything human）？任何身体部位（any body part）？任何被注意之物（anything attended）？任何带曲线之物（anything curvy）——也有反应？用 1-back 任务控制注意量。
...
**梭状回面孔区（fusiform face area, FFA）**：在几乎每个正常被试相近位置都能找到；对人脸的反应高于迄今测试过的任何其它刺激。下表是 FFA 对各类刺激的相对 BOLD 反应强度（数值越大反应越强）。可以看到一条由强到弱的梯度：正立真人脸最强，类人脸的图（卡通、猫脸、残缺人脸）居中，纯非人脸物体（手、建筑）最弱。
...
缺点：无法判断所测活动是否在认知/行为中起因果作用；BOLD 的生理基础不清楚（突触活动还是动作电位 synaptic activity vs spikes）；空间分辨率最好约 1 mm（难看清皮层柱 cortical columns）；昂贵（> \$600/小时）；耳道与鼻窦附近磁不均匀导致的磁敏感伪影（susceptibility artifact）；噪声大；时间分辨率远不及视觉信息加工的时间尺度。

**为什么要问"多快"**：因为我们想理解识别人脸时脑内运行的计算。有些计算是迭代的、含反复的假设检验与反馈（iterative, feedback, generative models），有些则是沿视觉系统的一次前馈扫描（a feedforward sweep）——二者时间尺度可能很不同。fMRI 回答不了这个问题，因为它经由血流、太慢。

---

# D. 事件相关电位（ERPs）

**脑电图（electroencephalography, EEG）**：12–100+ 个头皮电极测量脑的电反应，对大量神经元求和。空间分辨率很差。

旧类比：像把麦克风放在橄榄球场顶棚内侧——能知道何时达阵（touchdown）得分，但仅此而已；很难分辨看台上某人对另一人说了什么。（这一点正在改变。）

**Thorpe et al (1996)（指定阅读）**：人能多快判断一张图里是否有动物？

为什么不直接用反应时（reaction time）？因为反应时把运动阶段也算进去了——看到动物 → 判断按哪个键 → 用哪根手指 → 信号沿传导速度一路传到手指，这段运动耗时与知觉耗时混在一起。于是该研究"从脑内神经元那里读出一个反应时"：对额叶（frontal）电极的 ERP 求平均，有动物与无动物两条曲线在约 150 ms 处分叉。

⭐ 推理很微妙：曲线在 150 ms 分叉，说明到 150 ms 时脑内对"有/无动物"已开始有所不同。但 150 ms 既是该加工发生时刻的**上界**（可能更早就完成、我们看到的是更晚的运动阶段），也是该加工**起始**时刻的上界（分叉≠加工已完成）。

**ERP = EEG 时间锁定到刺激起始（time-locked to stimulus onset）、再跨试次信号平均**，故名"事件相关电位"（Event-Related Potential）。

**Bentin et al (1996)**：刺激起始后 170 ms 出现人脸特异反应（即 N170），相对汽车、打乱的人脸等，在大致枕颞位置出现更大的波峰，右半球更明显。说明：(1) 又一项证据表明脑中存在对人脸特异的机制；(2) 人脸在刺激后 170 ms 已被（开始）与非人脸区分（faces are discriminated from nonfaces by 170 ms，很快）。但该信号是否来自 FFA？无从得知——大概在脑后部（枕颞电极更明显），源定位是病态的。

---

# E. 脑磁图（MEG）

头部周围排布 300+ 个传感器，测量神经元电流产生的磁场（用右手定则 right-hand rule）。

**为什么主要"看见"脑沟**：支撑知觉/认知的活动多发生在灰质，电流方向大致垂直穿过皮层。位于脑回（bumps, gyri）的活动，按右手定则其磁场大体留在皮层内，外部探测器难以测到；而位于脑沟（folds, sulci）的活动，其磁场会伸出脑外，可被磁传感器探测。故 MEG 主要"看见"脑沟内、即皮层中与头皮垂直部分的活动，而非脑回。
...
⭐ 这是人身上唯一同时具备高空间与高时间分辨率的方法。对象是难治性癫痫（intractable epilepsy）的神经外科患者：取下一块颅骨，切开并拨开硬脑膜（dura mater），把电极直接贴在脑表面（subdural）。这样做有两个临床目的：当患者发作时三角定位癫痫灶（seizure focus）；以及绘制功能图（避免切到语言、运动等区域）。部分患者愿意在此期间配合看实验刺激。

**脑表面记录（日本患者，沿梭状回的电极条）**：约 2 mm 大小的皮层小块对人脸近乎排他地反应，约 150 ms 起反应；该反应极其选择性，远比 fMRI 更选择（因直接记自脑表面），且自带时间信息——反应约在 130 ms 起、约 170 ms 达峰。2 mm 电极尺寸接近一个 fMRI 体素，但模糊更少（fMRI 因血流而空间模糊）；尽管如此，仍是对数万个神经元求平均（比 fMRI 的数十万个少一个量级）。
...
⭐ **单一分离（single dissociation）不足以下结论**：仅有面孔失认这一现象，仍与"人脸识别只是比物体识别更难"相容——若被损坏的是通用物体识别系统的一部分，较难的人脸任务会受更大冲击。因此不能仅凭面孔失认就断定该区"专司人脸识别"。
...
分辨率给不了因果性；要测因果作用就得去扰动它。神经外科医生有时通过那些贴在脑表面的同一批电极进行电刺激（用于测试该区功能、定位癫痫灶）。在这类罕见情形下，可在科学上"鱼与熊掌兼得"。
...
| 患者脑损伤（Lesion） | 受损伤范围限制 | — | —（已存在的损伤） | 是（尤其双重分离） |
